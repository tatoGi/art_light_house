                ...
                <form action="{{ route('categories.update', [app()->getlocale() , $category->id]) }}" method="POST" enctype="multipart/form-data" data-parsley-validate novalidate>
                    ...
                    <div class="w-1/2 relative bg-white mt-2 mb-2 rounded-md p-4 mx-auto">
                        ...
                        <div class="flex s justify-center items-center flex-col mb-2">
                            <label for="category" class="text-sm font-medium text-gray-900 dark:text-gray-400">Main Category</label>
                            ...
                            @foreach (config('app.locales') as $locale)
                                <div id="locale-{{ $locale }}" class="@if ($locale !== app()->getLocale()) hidden @endif p-4">
                                    <div class="flex flex-col w-full items-center justify-center mb-2">
                                        <label for="title_{{ $locale }}" class="text-sm font-medium"><span class="text-red-500">*</span>Title ( {{ __('admin.locale_' . $locale) }})</label>
                                        <input type="text" name="{{ $locale }}[title]" id="title_{{ $locale }}" value="{{$category->translate($locale)->title ?? '' }}" class="border w-full text-sm rounded-lg block p-2.5 @error('title_' . $locale) border-red-500 @enderror" placeholder="Title">
                                        @error('title_' . $locale)
                                    ...
                                    <div class="flex w-fulls items-center justify-center flex-col mb-2">
                                        <label for="slug_{{ $locale }}" class="text-sm font-medium"><span class="text-red-500">*</span>URL Keyword ( {{ __('admin.locale_' . $locale) }})</label>
                                        <input type="text" name="{{ $locale }}[slug]" id="slug_{{ $locale }}" value="{{$category->translate($locale)->slug ?? '' }}" class="border w-full text-sm rounded-lg block p-2.5 @error('slug_' . $locale) border-red-500 @enderror" placeholder="URL Keyword">
                                        @error('slug_' . $locale)
                                            <span class="text-red-500 text-sm">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="flex w-full items-center justify-center flex-col mb-2">
                                        <label for="description_{{ $locale }}" class="text-sm font-medium text-gray-900 dark:text-gray-900"><span class="text-red-500">*</span>Description ( {{ __('admin.locale_' . $locale) }})</label>
                                        <div id="editor" class="editor">
                                            <textarea id="description_{{ $locale }}" name="{{ $locale }}[description]" class="border w-full ckeditor text-sm text-gray-900 rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-blue-500 dark:focus:border-blue-500 p-2.5 @error('description_' . $locale) border-red @enderror" placeholder="Description">{!! $category->translate($locale)->description ?? '' !!}</textarea>
                        ...
                        <div class="flex w-full mb-2">
                            <div class="flex flex-col mb-2 mt-2">
                                <div class="flex w-full bg-grey-lighter">
                                    <label id="upload-label" class="w-64 flex flex-col items-center px-4 py-6 bg-white text-blue rounded-lg shadow-lg tracking-wide uppercase border border-blue cursor-pointer hover:bg-blue ">
                                        <svg class="w-8 h-8" fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                                            <path d="M16.88 9.1A4 4 0 0 1 16 17H5a5 5 0 0 1-1-9.9V7a3 3 0 0 1 4.52-2.59A4.98 4.98 0 0 1 17 8c0 .38-.04.74-.12 1.1zM11 11h3l-4-4-4 4h3v3h2v-3z" />
                                        </svg>
                                        <span id="upload-text" class="mt-2 text-base leading-normal">Select a file</span>
                                        <input id="upload-input" type='file' name="icon" class="hidden" onchange="uploadFile(this)" />
                                    </label>
                                    <div id="image-container" class="w-full h-full pl-3">
                                        @if($category->icon)
                        ...
                        <div class="flex w-1/2 flex-col mb-2">
                            ...
                            <label class="relative inline-flex cursor-pointer items-center">
                                <input id="switch" type="checkbox" class="peer sr-only" name="active" value="1" {{ $category->active == 1 ? 'checked' : '' }} />
